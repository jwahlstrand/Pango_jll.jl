# Autogenerated wrapper script for Pango_jll for x86_64-apple-darwin14
export libpangoft, libpangocairo, libpango
...
const libpangoft_splitpath = ["lib", "libpangoft2-1.0.0.dylib"]
...
const libpangoft = "@rpath/libpangoft2-1.0.0.dylib"


# Relative path to `libpangocairo`
const libpangocairo_splitpath = ["lib", "libpangocairo-1.0.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libpangocairo = "@rpath/libpangocairo-1.0.0.dylib"


# Relative path to `libpango`
const libpango_splitpath = ["lib", "libpango-1.0.0.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libpango = "@rpath/libpango-1.0.0.dylib"
